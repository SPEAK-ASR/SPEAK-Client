...
export function TranscriptionView({
    ...
    const copyAllTranscriptions = async () => {
        ...
        try {
            await navigator.clipboard.writeText(
                `${videoMetadata?.title || "YouTube Video"} - Transcriptions\n\n${allText}`,
            );
    ...
    const handleDeleteFailedTranscriptions = async () => {
        const nullCount = transcriptions.filter(
            (clip) => !clip.transcription || clip.transcription.trim() === "",
        ).length;
        ...
        try {
            ...
            if (response.success) {
                alert(
                    `Successfully deleted ${response.total_deleted} audio files with null transcriptions.\n\n${response.remaining_clips} clips remaining.`,
                );
            ...
            alert(
                "Failed to delete null transcriptions. Please check the console for details.",
            );
        } finally {
            setIsDeleting(false);
        }
    ...
    const handleRevert = async () => {
        ...
        try {
            ...
            alert(
                "Failed to delete files. Please check the console for details.",
            );
        } finally {
    ...
    const nullTranscriptionCount = transcriptions.filter(
        (clip) => !clip.transcription || clip.transcription.trim() === "",
    ).length;
                    ...
                    <div className="flex gap-2">
                        ...
                        {nullTranscriptionCount > 0 && (
                            ...
                            >
                                ...
                                {isDeleting
                                    ? "Deleting..."
                                    : `Delete Failed (${nullTranscriptionCount})`}
                            </button>
                        )}
                        <button
                ...
                <div className="space-y-4 max-h-[300px] overflow-y-auto">
                    {transcriptions.map((clip) => {
                        ...
                        const displayText = isExpanded
                            ...
                            : truncateText(clip.transcription);
                        const needsTruncation =
                            clip.transcription &&
                            clip.transcription.trim() !== "" &&
                            clip.transcription.length > 150;
