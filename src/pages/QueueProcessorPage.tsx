...
import { VideoQueueInput, BatchSettings, VideoQueueTable } from '../components/queue';
...
export function QueueProcessorPage() {
    ...
    const [defaultSettings, setDefaultSettings] = useState<VideoSettings>(DEFAULT_VIDEO_SETTINGS);
    ...
    const handleLoadPlaylist = useCallback(async (playlistUrl: string, limit?: number) => {
        setIsLoadingPlaylist(true);
        try {
            const response = await audioApi.getPlaylistVideos(playlistUrl, limit);
            if (response.success && response.videos.length > 0) {
                const newVideos: QueueVideo[] = response.videos.map((video: PlaylistVideo) => ({
                    id: generateId(),
                    url: video.url,
    ...
    const handleUpdateSettings = useCallback((id: string, settings: VideoSettings) => {
        setQueue((prev) =>
    ...
    const updateVideo = useCallback((id: string, updates: Partial<QueueVideo>) => {
        setQueue((prev) =>
    ...
    const processVideo = useCallback(async (video: QueueVideo) => {
        const { id, url, settings } = video;

        try {
            // Stage 1: Split Audio
            updateVideo(id, { status: 'splitting', progress: STAGE_PROGRESS.splitting });
            ...
            // Stage 2: Transcribe
            updateVideo(id, { status: 'transcribing', progress: STAGE_PROGRESS.transcribing });
            const transcribeResult = await audioApi.transcribeClips(splitResult.video_id);
            ...
            if (settings.autoCleanNullTranscriptions) {
                updateVideo(id, { status: 'cleaning', progress: STAGE_PROGRESS.cleaning });
                await audioApi.cleanNullTranscriptions(splitResult.video_id);
            }
            ...
            const saveResult = await audioApi.saveToCloud(splitResult.video_id);
    ...
    useEffect(() => {
        ...
        const processNext = async () => {
            ...
            const processing = queue.filter((v) =>
                ['splitting', 'transcribing', 'cleaning', 'saving'].includes(v.status)
            );

            // If we have capacity, start more videos
            if (processing.length < MAX_CONCURRENT) {
                const pending = queue.filter((v) => v.status === 'pending');
                const toStart = pending.slice(0, MAX_CONCURRENT - processing.length);

                for (const video of toStart) {
                    processVideo(video);
                }
            }
            ...
            const hasActive = queue.some((v) =>
                ['pending', 'splitting', 'transcribing', 'cleaning', 'saving'].includes(v.status)
            );
        ...
        processNext();
    }, [isProcessing, queue, processVideo]);
    ...
    const hasVideosToProcess = queue.some((v) => v.status === 'pending' || v.status === 'error');
        ...
        <Box sx={{ minHeight: '100vh', p: 2 }}>
            <Box sx={{ maxWidth: 1400, mx: 'auto' }}>
                ...
                <Box sx={{ textAlign: 'center', mb: 3 }}>
                    <Typography variant="h3" component="h1" fontWeight="bold" sx={{ mb: 0.5, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                        ...
                        {isProcessing && (
                            <Alert severity="info" sx={{ mb: 2 }}>
                                Processing up to {MAX_CONCURRENT} videos concurrently. You can stop at any time.
                            </Alert>
                        )}
