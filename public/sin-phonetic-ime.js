...
(function () {
  ...
  function attachIME(textarea, opts = {}) {
    ...
    const chip = resolveElement(opts.chip, opts.chipId || 'imeChip');
    const toggle = resolveElement(opts.toggle, opts.toggleId || 'imeToggle');
    ...
    textarea.addEventListener('beforeinput', beforeInputHandler);
    ...
    if (chip) {
      ...
      listeners.push(() => chip.removeEventListener('click', chipHandler));
    }
    ...
    if (toggle) {
      ...
      listeners.push(() => toggle.removeEventListener('change', toggleHandler));
    }
